...
router_compte = APIRouter(prefix="/compte", tags=["compte"])
...
class CompteRegisterReturnModel(BaseModel):
    email: str | None
    date_inscription: datetime.datetime | None
    type_compte: int | None
...
@router_compte.get(
    "/",
    response_model=List[CompteModel],
...
@router_compte.get(
    "/view/{id}", response_model=CompteModel, summary="Returns a single account"
)
